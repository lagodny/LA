...
var
  ...
  Comp, LookupRoot, OldLookupRoot: TComponent;

  procedure ReadProperty;
  begin
    PropName := R.ReadStr;

    if (Assigned(FComparer) and FComparer(CompClass, CompName, PropName)) or
      (not Assigned(FComparer) and ContainsText(PropName, 'color')) then
//    if ContainsText(PropName, 'color') then
...
//      R.SkipValue;
  ...
  procedure ReadComponent;
  begin
    OldLookupRoot := LookupRoot;
    try
      R.ReadPrefix(Flags, Position);
      CompClass := R.ReadStr;
      CompName := R.ReadStr;
      if LookupRoot.Name = CompName then
        Comp := LookupRoot
      else
      begin
        Comp := LookupRoot.FindComponent(CompName);
        if ffInline in Flags then
          LookupRoot := Comp;
      end;
      ReadData;
    finally
      LookupRoot := OldLookupRoot;
    end;
  end;
