...

{
  *****************************************************************************
  - TDataUpdater - компонент, который содержит
    - список линков к наблюдател€м (ƒатчики, “рекеры и прочее: линк - это
      по сути адаптер к наблюдателю, который знает как и что у наблюдател€
      нужно изменить при изменении объекта наоблюдени€)
    - подключение к серверу ћониторинга
    - поток, который периодически выполн€ет запросы к серверу и уведомл€ет наблюдаетелей об изменени€х
  - Ќаблюдатели должны подписыватьс€ на уведомлени€, при этом создаетс€ линк - TDCLink
...
type
  ...
  /// <summary>
  ///  ќбъект, который получает данные из ћониторинга и уведомл€ет подписчиков
  /// </summary>
  ...
  private
    const
      ...
      DefInterval = 1000;
    type
      // поток дл€ получени€ данных с сервера ћониторинга
...
begin
  // нельз€ изменить коннктор, во врем€ работы потока
  if Active then
    Exit;
