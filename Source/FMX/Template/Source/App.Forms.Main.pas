...
type
  ...
  private
    ...
    function SetupView<T: TFrame>(const aCloseExceptions, aHideExceptions: TArray<TClass>; aParent: TFmxObject;
      aAction: TAction; aProc: TProc<TObject> = nil; aEnableMainMV: Boolean = False): TFrameInfo<T>;
  private
...
//
//
//  for var aFrameInfo in FrameStand1.FrameInfos.Values do
//  begin
//    if (aFrameInfo.Frame is TBaseFrame) then
//      TBaseFrame(aFrameInfo.Frame).LoadSettings;
//  end;
//  Settings.DoChange;
end;
...
begin
  try
    ...
    begin
      if (aFrameInfo.Frame is TBaseFrame) then
        if aFrameInfo.IsVisible then
          TBaseFrame(aFrameInfo.Frame).SaveSettings;
    end;
...
begin
  ...
  begin
    ...
    for var aFrameInfo in FrameStand1.FrameInfos.Values do
    begin
      if aFrameInfo.IsVisible and (aFrameInfo.Frame is TBaseFrame) then
      begin
        f := TBaseFrame(aFrameInfo.Frame);
        if f.Back then
        begin
          Key := 0;
          KeyChar := #0;
...
begin
  ...
  begin
    if (aFrameInfo.Frame is TBaseFrame) then
      TBaseFrame(aFrameInfo.Frame).HandleApplicationEvent(AAppEvent, AContext);
  end;
...
begin
  LFrameInfos := FrameStand1.FrameInfos.Values.ToArray;
...
begin
  case aEvent.Value.Command of
    mcShowLogin:
      SetupView<TLoginFrame>([], [], loMainScrolable, nil, nil, False);
    mcHome:
      SetupView<THomeFrame>([], [], loMainContent, aHome, nil, True);

//      SetupView<TLoginFrame>([], [], loMainContent, nil, nil, False);
//    mcSignUp:
...
//      TDCLog.WriteToLog('mcShowInterval done');
    ...
    mcShowSettings:
    begin
      SetupView<TSettingsFrame>([THomeFrame], [], loMainContent, aSettings, nil, True);
    end;
...
begin
  ...
  begin
    if (aFrameInfo.Frame is TBaseFrame) then
      if aFrameInfo.IsVisible then
        TBaseFrame(aFrameInfo.Frame).OrientationChanged(ASender, AMessage);
  end;
end;
...
//  FrameStand1.HideAndCloseAllExcept(aCloseExceptions + [T]);
  FrameStand1.CloseAllExcept(aCloseExceptions + [T]);
//  Result := FrameStand1.GetFrameInfo<T>(True, MainLayout, '');// 'fader');
  Result := FrameStand1.FrameInfo<T>;
  if Result = nil then
  begin
    aIsNew := True;
//    Result := FrameStand1.GetFrameInfo<T>(True, aParent, '');// 'fader');
    Result := FrameStand1.New<T>(aParent, '');// 'fader');
  end
  else
    aIsNew := False;

  if Result.Frame is TBaseFrame then
  begin
    TBaseFrame(Result.Frame).Proc := aProc;
    TBaseFrame(Result.Frame).ApplySystemBarsInsets(SystemBars);
  end;

  if Result.Frame is TToolbarFrame then
  begin
//    mvMain.Parent := TToolbarFrame(Result.Frame); // loContent;

    if TToolbarFrame(Result.Frame).sbDrawer.Visible then
      mvMain.MasterButton := TToolbarFrame(Result.Frame).sbDrawer
    else
      mvMain.MasterButton := nil;
...
//    TToolbarFrame(Result.Frame).loTop.Margins.Top := SystemBars.Insets.Bottom;
  end;
...
//
  ...
  if aIsNew and (Result.Frame is TBaseFrame) then
    TBaseFrame(Result.Frame).DoAfterCreate;
...
//  Padding.Rect := SystemBars.Insets;
  ...
  begin
    if (aFrameInfo.Frame is TBaseFrame) then
      if aFrameInfo.IsVisible then
        TBaseFrame(aFrameInfo.Frame).ApplySystemBarsInsets(SystemBars);
  end;
