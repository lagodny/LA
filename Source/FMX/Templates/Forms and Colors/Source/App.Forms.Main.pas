...
uses
  System.SysUtils, System.Types, System.UITypes, System.Classes, System.Variants, System.Actions, System.Messaging, System.IOUtils,
  ...
  LA.FMX.UI.Consts,
  App.Consts, App.Events, LA.FMX.Colors, LA.FMX.StdCtrls;
...
//  ShowMessage('Settings changed');
  ...
  if LAColorManager1.Scheme <> Settings.Scheme then
  begin
    LAColorManager1.Scheme := Settings.Scheme;
    LAColorManager1.Apply;
    var f := System.IOUtils.TPath.Combine(Settings.GetAppFilesPath, 'LAStyle.style');
    TStyleManager.SetStyleFromFile(f);
//    DMCommon.SVGImages.RefreshAllIcons;
//    TStyleManager.UpdateScenes;
//    ShowMessage(LAAlphaColorToString( onPrimaryFixedVariant))
...
begin
  StyleBook := nil;
  if LAColorManager1.Scheme = 'dark' then
    LAColorManager1.Scheme := 'light'
  else
    LAColorManager1.Scheme := 'dark';
  LAColorManager1.Apply;
//  TStyleManager.UpdateScenes;
//  //StyleBook
//  //DMCommon.
//  DMCommon.MainStyleBook.LoadFromFile('DefStyle.style');
  var f := System.IOUtils.TPath.Combine(Settings.GetAppFilesPath, 'LAStyle.style');
  if not FileExists(f) then
    ShowMessage('Not Exists: '+ f);
//    ShowMessage('Exists: '+ f)
//  else


  TStyleManager.SetStyleFromFile(f);
  DMCommon.SVGImages.RefreshAllIcons;
//  DMCommon.MainStyleBook.
end;
