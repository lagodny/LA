...
function TSettings.GetAppFilesPath: string;
begin
  {$IFDEF WINDOWS}
  Result := '';
  {$ENDIF}
...
begin
  ...
  try
    ...
    try
      ...
      aVal := aJson.GetValue('scheme');
      if Assigned(aVal) and (aVal is TJSONObject) then
      begin
        aObj := aVal as TJSONObject;
        Scheme := aObj.GetValue<string>('name', 'light');
      end;
...
//        aArr.Add(FMapDevices[i]);
      ...
      aObj := TJSONObject.Create;
      aObj.AddPair('name', Scheme);
      aJson.AddPair('scheme', aObj);
